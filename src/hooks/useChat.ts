...
export function useChat() {
  ...
  const generateResponse = useCallback(async (
    ...
    const contextAtStart = messagesOverride || [...messages];
    ...
    try {
      ...
      // Safety check: Groq requires at least one message, and usually the last one must be from user
      if (apiMessages.length === 0) {
        observability.addLog('Inference', 'warn', "Empty message array detected. Injecting safety placeholder.", {});
        registerAutoFix("Injected safety placeholder to prevent empty messages array error.", "AF-227");
      ...
      for await (const chunk of stream) {
        if (chunk.raw) {
          updateSessionMessages(activeSessionId, prev => prev.map(m => 
            m.id === assistantMessageId ? { ...m, rawResponse: chunk.raw } : m
          ));
        }
      ...
      logRuntimeEvent({
        ...
        source: 'useChat.generateResponse',
  ...
  const createBranch = useCallback(async (
    ...
    if (originalMsg.role === 'user') {
      ...
      await generateResponse(apiKey, modelId, config, observability, assistantMessageId, undefined, undefined, undefined, [...messages, newMessage]);
    }
  }, [activeSession, activeSessionId, setSessions, updateSessionMessages, generateResponse, messages]);
  ...
  const sendMessageStream = useCallback(async (
    ...
    await generateResponse(apiKey, modelId, config, observability, assistantMessageId, onMetrics, onQuotaUpdate, files, updatedMessages);
