...
export const DataVault: React.FC = () => {
  ...
  const { showToast, githubToken, setGithubToken } = useStore();
  const { startGroup, addLog, startSubGroup, endGroup } = observability;
  const [githubUser, setGithubUser] = useState(() => secretsConfig.getGithubUser());
  const [githubRepo, setGithubRepo] = useState(() => secretsConfig.getGithubRepo());
  const [groqApiKey, setGroqApiKey] = useState(() => secretsConfig.getGroqApiKey() || '');
  const [openAiApiKey, setOpenAiApiKey] = useState(() => secretsConfig.getOpenAiApiKey() || '');
  ...
  const handleGithubTokenChange = async (val: string) => {
    setGithubToken(val);
    const cleanToken = val.trim().replace(/['"]/g, '');
    
    // Auto-detect valid token
    if (cleanToken && (cleanToken.startsWith('ghp_') || cleanToken.startsWith('github_pat_') || cleanToken.length > 30)) {
      secretsConfig.setGithubToken(cleanToken);
      const groupId = startGroup("Athanor Forja AF-191: Auto-Sync Deployment");
      try {
        await deploymentService.copyScript(observability, groupId, cleanToken);
  ...
  const generateScript = () => {
    ...
    const groupId = startGroup("Athanor Forja AF-191: Headless Pipeline Override");
  ...
  const exportWorkspace = () => {
    const groupId = startGroup("Athanor Forja AF-193: Universal Workspace Port (UWP) Deployment");
    ...
    const workspace = {
      ...
      auth: {
        ...
        openai_key: secretsConfig.getOpenAiApiKey() || '',
        github_token: secretsConfig.getGithubToken() || '',
        github_user: secretsConfig.getGithubUser(),
        github_repo: secretsConfig.getGithubRepo()
      },
      settings: {
    ...
    a.href = url;
    a.download = 'athanor_forja_workspace.json';
    a.click();
    ...
    reader.onload = (e) => {
      try {
        ...
        if (confirm("Atención: Esto sobreescribirá tu sesión actual. ¿Deseas inyectar este Workspace?")) {
          ...
          secretsConfig.setOpenAiApiKey(workspace.auth.openai_key);
          secretsConfig.setGithubToken(workspace.auth.github_token);
          if (workspace.auth.github_user) secretsConfig.setGithubUser(workspace.auth.github_user);
          if (workspace.auth.github_repo) secretsConfig.setGithubRepo(workspace.auth.github_repo);
  ...
  const exportTrainingDataset = () => {
    const groupId = startGroup("Athanor Forja AF-194: ML Dataset Export");
  ...
  const handleSaveCredentials = () => {
    ...
    const cleanOpenAi = openAiApiKey.trim().replace(/['"]/g, '');
    
    setGithubToken(cleanGithub);
    secretsConfig.setGithubUser(githubUser);
    secretsConfig.setGithubRepo(githubRepo);
    secretsConfig.setGroqApiKey(cleanGroq);
    secretsConfig.setOpenAiApiKey(cleanOpenAi);
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              className="w-full bg-zinc-900/50 border border-zinc-800 rounded-lg py-1.5 px-2 text-[10px] font-mono text-zinc-400 focus:outline-none focus:border-orange-500/50 transition-all"
            />
            <input
              type="text"
              value={githubRepo}
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between px-1">
              <label className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest">GitHub Token</label>
              <a 
