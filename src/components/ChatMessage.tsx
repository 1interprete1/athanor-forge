...
export function ChatMessage({ 
  ...
  return (
    <motion.div 
      ...
      className={`flex flex-col gap-1.5 w-full max-w-3xl mx-auto group/msg ${isMobile ? 'px-1' : ''}`}
    >
      <div className="flex items-center justify-between px-1">
        <div className={`font-mono text-[9px] font-medium tracking-tight uppercase flex items-center gap-2 ${isUser ? 'text-indigo-400' : 'text-cyan-400'}`}>
          <span className="text-zinc-500">[{displayTime}]</span>
          {isUser ? (
            <span>USR_SIG</span>
          ) : (
            <span>LPU_OUT {isMobile ? '' : <span className="text-zinc-600 ml-1">120B_ENGINE</span>}</span>
          )}
        </div>
      ...
      <div className={`relative ${isMobile ? 'pl-3' : 'pl-4'} border-l transition-all duration-500 ${
        isGenerating && !isUser 
        ...
        {isUser ? (
          isEditing ? (
            <div className={`space-y-3 ${isMobile ? 'min-w-0' : 'min-w-[300px]'} py-1`}>
              <textarea
                value={editContent}
              ...
              <div className="flex justify-start gap-2">
                ...
                >
                  <X className="w-3 h-3" /> {isMobile ? 'X' : 'Cancelar'}
                </button>
                <button 
                ...
                >
                  <Check className="w-3 h-3" /> {isMobile ? 'Save' : 'Guardar y Bifurcar'}
                </button>
              </div>
            </div>
          ) : (
            <div className="group relative py-1">
              ...
              {!isGenerating && (
                <button 
                  ...
                  className={`absolute ${isMobile ? '-right-2' : '-left-10'} top-0 p-1.5 rounded-md bg-white/5 border border-white/10 text-neutral-500 ${isMobile ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-all hover:text-white hover:bg-white/10`}
                  ...
                  components={{
                    p: ({ node, children, ...props }: any) => {
                      const hasBlock = node?.children?.some((child: any) => 
                        child.type === 'element' && 
                        ['pre', 'div', 'blockquote', 'ul', 'ol', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table'].includes(child.tagName)
                      );
                      if (hasBlock) {
                        return <div className="leading-relaxed mb-4" {...props}>{children}</div>;
                      }
        ...
        {!isUser && content && !isGenerating && (
          <div className={`mt-0.5 relative ${isMobile ? 'h-auto py-2 flex-wrap' : 'h-[20px]'} bg-zinc-900/40 flex items-center px-2 border-y border-zinc-800/50 overflow-hidden`}>
            <div className={`text-[8.5px] font-mono text-zinc-500 uppercase tracking-tight flex ${isMobile ? 'flex-wrap gap-x-3 gap-y-1' : 'items-center gap-3'} w-full`}>
              <span className={`font-semibold text-zinc-400 ${isMobile ? 'w-full mb-1' : ''}`}>LPU_STATS</span>
              {!isMobile && <span className="text-zinc-700">|</span>}
              <span>TPS: <span className="text-zinc-300">{tps?.toFixed(1) || '0.0'}</span></span>
              {!isMobile && <span className="text-zinc-700">|</span>}
              <span>TTFT: <span className="text-zinc-300">{ttft?.toFixed(0) || '0'}ms</span></span>
              {!isMobile && <span className="text-zinc-700">|</span>}
              <span className="flex items-center gap-1">
                COST: <span className="text-zinc-300">${cost ? cost.toFixed(5) : '0.00000'}</span>
                {!isMobile && (
                  <span className="text-zinc-600">
                    (MXN ${((cost || 0) * usdToMxnRate).toFixed(4)})
                  </span>
                )}
              </span>
              
              <div className={`${isMobile ? 'w-full mt-1 flex justify-end' : 'ml-auto'} flex items-center gap-3`}>
                {turnActions && (
                  <>
                    {turnActions}
                    {!isMobile && <span className="text-zinc-700">|</span>}
                ...
                {feedbackState === 'positive' ? (
                  <span className="text-emerald-500 font-semibold">ASSIMILATED</span>
                ) : feedbackState === 'negative' ? (
                  <span className="text-rose-500 font-semibold">POOR</span>
                ) : (
                  <>
                    <button 
