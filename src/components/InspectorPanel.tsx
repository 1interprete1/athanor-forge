import { Settings2, Terminal, Zap, RotateCcw, Shield, Radar, Bug, Download, Play } from 'lucide-react';
...
import { BugRegistry } from './BugRegistry';
import { BUG_REGISTRY } from '../system/bugRegistry';
import { ExportConfigPanel } from './ExportConfigPanel';
import { buildExportPayload, formatSessionForExport } from '../utils/exportHelper';
...
export function InspectorPanel() {
  const { config, updateConfig, resetConfig, activeModelId, models, metrics, estimatedTokens, stagedTokens, isPilotActive, quota, isGenerating, runContextAudit, sessions, activeSessionId, exportConfig, devMode, setDevMode, startReplay, replayState, showToast, isMobile } = useStore();
  ...
  const handleExportSession = () => {
    ...
    if (!activeSession || activeSession.messages.length === 0) {
      console.warn("[EXPORT_EMPTY_BLOCKED]");
      showToast("No content to export", "error");
      return;
    }

    const exportDataStr = formatSessionForExport(activeSession, exportConfig, devMode);

    const blob = new Blob([exportDataStr], { type: 'application/json;charset=utf-8' });
  ...
  return (
    <div className="h-full relative flex flex-col bg-neutral-950 text-neutral-200 overflow-hidden">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={() => {
              console.log('[InspectorPanel] Bug Registry button clicked. Current state:', showBugs);
          ...
          >
            <Bug className={`w-4 h-4 ${showBugs ? 'text-orange-400' : 'text-neutral-500'}`} />
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold uppercase tracking-widest">Bug Registry</span>
                {BUG_REGISTRY.filter(b => b.status !== 'resolved').length > 0 && (
                  <span className="bg-orange-500 text-black text-[8px] font-black px-1 rounded-full animate-pulse">
                    {BUG_REGISTRY.filter(b => b.status !== 'resolved').length}
                  </span>
                )}
              </div>
              <span className="text-[9px] truncate opacity-70">
                {showBugs ? 'Close Registry' : 'View Known Anomalies'}
              </span>
            </div>
          </button>

          <button
            onClick={() => startReplay(activeSessionId)}
            disabled={replayState.isActive || isGenerating || sessions.find(s => s.id === activeSessionId)?.messages.length === 0}
            className={`w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all border ${
          ...
          >
            <Play className={`w-4 h-4 ${replayState.isActive ? 'text-emerald-400' : 'text-neutral-500'}`} />
            <div className="flex flex-col min-w-0">
              <span className="text-[10px] font-bold uppercase tracking-widest">Replay Session</span>
              <span className="text-[9px] truncate opacity-70">
                {replayState.isActive ? 'Replay Active' : 'Turn-by-turn playback'}
              </span>
            </div>
          </button>
          ...
          <button
            ...
            className="w-full text-left p-3 rounded-xl flex items-center gap-3 transition-all border bg-white/5 border-white/5 hover:bg-white/10 text-neutral-400 hover:text-white"
          >
            <Download className="w-4 h-4 text-neutral-500" />
            <div className="flex flex-col min-w-0">
              <span className="text-[10px] font-bold uppercase tracking-widest">Export Session</span>
              <span className="text-[9px] truncate opacity-70">
    ...
    {/* Bug Registry Modal/Overlay */}
      {createPortal(
        <AnimatePresence>
          {showBugs && (
            <>
              {/* Backdrop to ensure visibility and focus */}
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              <motion.div
                ...
                className={`fixed ${isMobile ? 'inset-x-0 bottom-0 h-[80vh] rounded-t-3xl' : 'inset-y-0 right-[280px] w-[400px]'} bg-neutral-950 border-l border-zinc-900 shadow-2xl z-[100] flex flex-col`}
              >
                <div className="p-4 border-b border-zinc-900 flex items-center justify-between bg-black/40">
                  <div className="flex items-center gap-2">
                    <Bug className="w-4 h-4 text-orange-500" />
                    <h2 className="text-xs font-bold uppercase tracking-widest text-white">Anomalies & Bugs</h2>
                  </div>
                  <button 
                    onClick={() => setShowBugs(false)}
                    className="p-1 hover:bg-white/10 rounded transition-colors"
                  >
                    <Radar className="w-4 h-4 text-zinc-500 rotate-45" />
                  </button>
                </div>
