...
import { TurnUnit } from './components/TurnUnit';
  ...
  return (
    <ForgeLoggerProvider>
      <StoreProvider>
        ...
        {!user && (
          <AuthGateway 
            onAuthenticated={(authenticatedUser, groqKey, openAiKey) => {
              setUser(authenticatedUser);
              // API keys are already handled in AuthGateway via secretsConfig
            }} 
          />
        )}
      </StoreProvider>
...
function AthanorForgeCore({ 
  ...
  const [apiKey, setApiKey] = useState<string | null>(() => secretsConfig.getGroqApiKey());
  const [openAiApiKey, setOpenAiApiKey] = useState<string | null>(() => secretsConfig.getOpenAiApiKey());
  const [editingSessionId, setEditingSessionId] = useState<string | null>(null);
  const [editingSessionTitle, setEditingSessionTitle] = useState("");
  const [sessionToDelete, setSessionToDelete] = useState<string | null>(null);

  // Update keys when user changes (e.g. after AuthGateway finishes)
  useEffect(() => {
    if (user) {
      setApiKey(secretsConfig.getGroqApiKey());
      setOpenAiApiKey(secretsConfig.getOpenAiApiKey());
    }
  }, [user]);
  ...
  useEffect(() => {
    startGroup(`Athanor Forja ${ATHANOR_VERSION}: Monolithic Shell Deployment`);
  ...
  useEffect(() => {
    const auditGroup = actions.startGroup("Athanor Forja AF-196: Visual & UX Sensory Mapping");
  ...
  // Google Drive Sync Logic (Moved after useChat to ensure sessions/setSessions are available)
  const syncToDrive = useCallback(async () => {
    ...
    try {
      const data = {
        ...
        artifacts,
        apiKeys: {
          groq: secretsConfig.getGroqApiKey(),
          openai: secretsConfig.getOpenAiApiKey(),
          githubUser: secretsConfig.getGithubUser(),
          githubRepo: secretsConfig.getGithubRepo(),
      ...
      try {
        ...
        addLog('System', 'success', 'Workspace synced to Google Drive', {});
      } catch (driveError: any) {
        if (driveError.message && driveError.message.includes('401')) {
          sessionStorage.removeItem('google_drive_token');
        }
        // Fallback to local storage if Drive fails (e.g. API not enabled)
        localStorage.setItem('athanor_workspace_backup', JSON.stringify(data));
        if (driveError.message && driveError.message.includes('Drive API Disabled')) {
          addLog('System', 'info', 'Google Drive API is disabled. Workspace saved locally.', {});
        } else {
          addLog('System', 'warn', 'Drive sync failed, saved locally instead', { error: driveError });
        }
      }
    } catch (error) {
      addLog('System', 'error', 'Failed to prepare workspace sync', { error });
    } finally {
      setIsSyncing(false);
    }
  }, [user, sessions, performanceHistory, favoriteModels, config, exportConfig, exportProfile, devMode, selectedModelId, layout, isObservabilityOpen, artifacts, addLog, isSyncing]);

  const loadFromDrive = useCallback(async () => {
    if (!user) return;
    
    const groupId = startGroup('Google Drive: Syncing Workspace...');
    try {
      let data = null;
      try {
        ...
        if (driveError.message && driveError.message.includes('Drive API Disabled')) {
          addLog('System', 'info', 'Google Drive API is disabled. Loading local backup.', {});
        } else {
          addLog('System', 'warn', 'Drive load failed, attempting local backup', { error: driveError });
        }
      ...
      if (data) {
        ...
        if (data.exportProfile) setExportProfile(data.exportProfile);
        if (data.devMode) setDevMode(data.devMode);
        if (data.activeModelId) setSelectedModelId(data.activeModelId);
        if (data.layout) setLayout(data.layout);
        if (data.isObservabilityOpen !== undefined) setIsObservabilityOpen(data.isObservabilityOpen);
        if (data.artifacts) setArtifacts(data.artifacts);
        
        // Add more state restoration as needed
        addLog('System', 'success', 'Workspace restored successfully', { groupId });
        showToast('Workspace restaurado', 'success');
      } else {
        addLog('System', 'info', 'No workspace found to restore', { groupId });
      }
    } catch (error) {
      addLog('System', 'error', 'Failed to restore workspace', { error, groupId });
      showToast('Error al restaurar workspace', 'error');
    } finally {
      endGroup();
    }
  }, [user, setSessions, setConfig, setExportConfig, setExportProfile, setDevMode, setSelectedModelId, setLayout, setIsObservabilityOpen, setArtifacts, addLog, startGroup, endGroup, showToast]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      syncToDrive();
    }, 10000); // Sync every 10 seconds after changes
    
    return () => clearTimeout(timer);
  }, [user, sessions, performanceHistory, favoriteModels, config, exportConfig, exportProfile, devMode, selectedModelId, layout, isObservabilityOpen, artifacts, githubToken, syncToDrive]);
  ...
  useEffect(() => {
    if (replayState.isActive && devMode === 'advanced' && exportProfile === 'audit') {
      setLayout(prev => ({ ...prev, rightPanelOpen: true }));
    }
  }, [replayState.isActive, devMode, exportProfile]);
  ...
  useEffect(() => {
    if (!auditRun.current) {
      ...
      const groupId = startGroup("Athanor Forja AF-221: HUD Synchronization & Predictive Fill");
  ...
  const processFile = async (file: File, id: string, parentGroupId: string) => {
    ...
    try {
      ...
      if (extension === 'pdf') {
        ...
        addLog('Vault', 'info', `Image Processor: Extracting metadata and generating preview for ${file.name}`, { subGroupId });
        ...
        setAttachedFiles(prev => prev.map(f => f.id === id ? { 
          ...
          preview, 
          status: 'ready',
          content: `[IMAGEN: ${file.name}]`, // Placeholder for text context
          estimatedTokens: 0 // Images are handled differently in vision models, but for text context we use 0
        } : f));
      ...
      // AF-226: Clean control characters from all file types
  ...
  useEffect(() => {
    scrollToBottom();
  }, [messages, replayState.currentTurnIndex]);

  useEffect(() => {
    if (auditRun.current) return;
    
    const init = async () => {
      const group = startGroup(`Athanor Forja ${ATHANOR_VERSION}: Gold Master - Final Synthesis & Certification`);
      auditRun.current = true;
  ...
  const handleCheckIntegrity = () => {
    const groupId = startGroup(`Athanor Forja ${ATHANOR_VERSION}: Integrity Status Check`);
  ...
  if (!apiKey || !openAiApiKey) {
    return null; // AuthGateway is handled at the root App component
  }
  ...
  return (
    <div className={`h-full transition-all ${isPilotActive ? 'duration-500' : 'duration-[2000ms]'} ${
        ...
        {sessionToDelete && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-[#0a0a0a] border border-zinc-800 p-6 max-w-sm w-full mx-4 shadow-2xl"
            >
              <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-2">Delete Experiment</h3>
              <p className="text-xs text-zinc-400 mb-6">Are you sure you want to delete this experiment? This action cannot be undone.</p>
              <div className="flex justify-end gap-3">
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 border border-red-500/20 text-xs font-bold uppercase tracking-widest transition-colors"
                >
                  Delete
                </button>
              </div>
      ...
      {isMobile && (
        <div className="fixed top-0 left-0 right-0 h-14 bg-[#050505]/80 backdrop-blur-xl border-b border-zinc-900 z-[60] flex items-center justify-between px-4">
          <div className="flex items-center gap-3">
            <ForgeLogo size={20} isStreaming={isGenerating} />
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-white uppercase tracking-widest leading-none">Athanor Forja</span>
              <span className="text-[8px] font-mono text-zinc-500 uppercase tracking-tighter">{ATHANOR_VERSION}</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
      ...
      sidebar={
        <div className="h-full flex flex-col bg-neutral-950/50 relative border-r border-white/5 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide relative z-10 flex flex-col">
            {/* Branding Block */}
            <div className="flex flex-col gap-1 p-4 relative z-20">
              <ForgeLogo size={32} isStreaming={isGenerating} />
              <div className="mt-1">
                <h1 className="text-[10px] font-bold tracking-tighter text-zinc-100 uppercase">ATHANOR FORJA</h1>
                <p className="text-[7px] font-mono text-zinc-500 tracking-[0.2em]">CERTIFIED_INFRASTRUCTURE</p>
              </div>
            </div>
            ...
            <div className="p-3 bg-black/20 relative z-20 space-y-2">
              <motion.button
                whileTap={{ y: 1, scale: 0.99 }}
                onClick={() => setIsModelSuiteOpen(true)}
                disabled={isPilotActive}
                className={`w-full text-left p-2 rounded-sm flex items-center gap-2 transition-all group border ${
                  isModelSuiteOpen 
              ...
              >
                <div className="relative flex items-center justify-center w-4 h-4">
                  <div className={`w-2.5 h-1 rounded-sm ${isGenerating ? 'bg-emerald-400 animate-pulse shadow-[0_0_4px_rgba(52,211,153,0.8),0_0_12px_rgba(52,211,153,0.4)]' : 'bg-emerald-500/40'}`} />
                </div>
                <div className="flex flex-col min-w-0">
                  <span className="text-[8px] font-bold uppercase tracking-widest">Active Engine</span>
                  <span className="text-[7px] truncate opacity-70">
            ...
            <div className="py-2 space-y-4">
              ...
              <div className="px-3 space-y-3">
                <div className="flex flex-col gap-1">
                  <button 
                    ...
                    disabled={isPilotActive}
                    className="w-full text-center py-1.5 border border-dashed border-zinc-800 text-zinc-500 hover:border-orange-500/50 hover:text-orange-500 hover:drop-shadow-[0_0_8px_rgba(249,115,22,0.5)] transition-all rounded-sm text-[9px] font-mono tracking-widest disabled:opacity-50"
                  >
                    [ NEW_EXPERIMENT ]
                  </button>
                </div>
                  ...
                  {sessions.map(session => (
                    <div key={session.id} className="relative group">
                      <button
                        ...
                        disabled={isPilotActive}
                        className={`w-full text-left p-2 rounded-none flex items-center gap-2 transition-colors border-b border-zinc-900/50 ${
                      ...
                      >
                        <MessageSquare className={`w-3 h-3 shrink-0 ${activeSessionId === session.id ? 'text-orange-400 drop-shadow-[0_0_8px_rgba(249,115,22,0.5)]' : 'text-neutral-500'}`} />
                        <div className="flex flex-col min-w-0 flex-1">
                          {editingSessionId === session.id ? (
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  ...
                                  if (editingSessionTitle.trim()) {
                                    handleRenameSession(session.id, editingSessionTitle.trim());
                                  }
                              ...
                              autoFocus
                              className="bg-black/50 border border-orange-500/50 text-[11px] font-medium text-white px-1 py-0.5 outline-none rounded-sm w-full"
                            />
                          ) : (
                          ...
                          <span className="text-[8px] font-mono opacity-50 truncate">
              ...
              {artifacts.length > 0 && (
                <div className="px-4 space-y-4">
                  <label className="text-[9px] font-bold text-neutral-600 uppercase tracking-widest ml-2 flex items-center gap-2">
                  ...
                  <div className="space-y-1">
                    {artifacts.filter(a => a.sessionId === activeSessionId && messages.some(m => m.id === a.messageId)).map(art => (
                      ...
                      >
                        ...
                        <span className="text-[10px] truncate">{art.title}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="flex-1 bg-neutral-950/20">
              <div className="border-b border-zinc-900">
                <button 
                  ...
                  disabled={isPilotActive}
                  className={`w-full flex items-center justify-between p-4 text-[9px] font-mono tracking-widest transition-colors ${
                ...
                {openAccordion === 'vault' && 
                  <div className="p-4 pt-0 space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <label className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest">OpenAI API Key</label>
                        <div className="relative">
                          <input 
                            type="password"
                            defaultValue={secretsConfig.getOpenAiApiKey() || ''}
                            onBlur={(e) => {
                              const val = e.target.value.trim();
                              if (val) {
                                secretsConfig.setOpenAiApiKey(val);
                                setOpenAiApiKey(val);
                                addLog('Security', 'success', "OpenAI API Key updated in Vault", {});
                              }
                            }}
                            className="w-full bg-black/40 border border-zinc-900 rounded-none p-2 text-[10px] font-mono text-zinc-400 focus:outline-none focus:border-orange-500/50"
                            placeholder="sk-..."
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-2">
                            <label className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest">GitHub User</label>
                            <input 
                              type="text"
                              defaultValue={secretsConfig.getGithubUser()}
                              onBlur={(e) => {
                                const val = e.target.value.trim();
                                if (val) {
                                  secretsConfig.setGithubUser(val);
                                  addLog('Security', 'success', `GitHub User updated to ${val}`, {});
                                }
                              }}
                              className="w-full bg-black/40 border border-zinc-900 rounded-none p-2 text-[10px] font-mono text-zinc-400 focus:outline-none focus:border-orange-500/50"
                          ...
                          <div className="space-y-2">
                            <label className="text-[8px] font-mono text-zinc-600 uppercase tracking-widest">GitHub Repo</label>
                            <input 
                              type="text"
                              defaultValue={secretsConfig.getGithubRepo()}
                              onBlur={(e) => {
                                const val = e.target.value.trim();
                                if (val) {
                                  secretsConfig.setGithubRepo(val);
                                  addLog('Security', 'success', `GitHub Repo updated to ${val}`, {});
                                }
                              }}
                              className="w-full bg-black/40 border border-zinc-900 rounded-none p-2 text-[10px] font-mono text-zinc-400 focus:outline-none focus:border-orange-500/50"
                              placeholder="Repo"
                        ...
                        <div className="space-y-2">
                          ...
                          <a 
                            href="https://github.com/settings/tokens/new?description=Athanor%20Forge%20Deployment&scopes=repo" 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="text-[8px] font-mono text-orange-500/60 hover:text-orange-500 flex items-center gap-1 transition-colors"
                          >
                        ...
                        <div className="relative">
                          <input 
                            ...
                            onChange={async (e) => {
                              const val = e.target.value.trim();
                              // Detect if it's a real token (not the obfuscated one)
                              if (val && val !== '********' && (val.startsWith('ghp_') || val.startsWith('github_pat_') || val.length > 30)) {
                                secretsConfig.setGithubToken(val);
                                addLog('Security', 'success', "GitHub Token auto-detected and secured", {});
                                
                                // Auto-copy script
                                const groupId = startGroup("Athanor Forja AF-182: Auto-Sync Deployment");
                                try {
                                  await deploymentService.copyScript({ addLog, startGroup, startSubGroup, endGroup } as any, groupId, val);
                                  showToast("Token guardado y Script copiado al portapapeles", "success");
                                } catch (err: any) {
                                  showToast("Token guardado, pero error al copiar script", "info");
                                }
                                endGroup();
                              }
                            }}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <button
                            onClick={async () => {
                              const groupId = startGroup("Athanor Forja AF-182: Deployment Pipeline Sync (Silent)");
                              try {
                                ...
                                showToast("Error al copiar el script", "error");
                              }
                              endGroup();
                            }}
      ...
      stage={
        ...
        >
          ...
          <div 
            ...
            className={`flex-1 overflow-y-auto ${isMobile ? 'pt-4 pb-4 px-2' : 'pt-8 pb-8 px-4'} scroll-smooth`}
          >
            {messages.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <div className="text-center space-y-4">
                  <div className="w-12 h-12 bg-transparent flex items-center justify-center mx-auto">
                    <ForgeLogo size={32} isStreaming={isGenerating} />
                  </div>
                  <div className="space-y-1">
                    <h2 className="text-lg font-bold text-white tracking-tight">Athanor Forja</h2>
                    <p className="text-xs text-neutral-500 font-medium">Inferencia Industrial & Sabiduría Técnica</p>
                  </div>
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-zinc-900/50 border border-zinc-800/50">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500/50 animate-pulse" />
                    <span className="text-[10px] font-mono text-zinc-400">
                      Connected to: <span className="text-orange-400/80">{activeModel?.id || selectedModelId || 'openai/gpt-oss-120b'}</span>
                    </span>
                  </div>
                </div>
              </div>
            ) : (
              <div className={`max-w-3xl mx-auto ${isMobile ? 'space-y-4' : 'space-y-5'}`}>
                {(!messages || !Array.isArray(messages)) ? null : (() => {
                  const turns = groupMessagesIntoTurns(messages);

                  const turnsToRender = replayState.isActive 
                    ? turns.slice(0, replayState.currentTurnIndex + 1) 
                    : turns;

                  return turnsToRender.map((turn, i) => {
                    const isLastTurn = i === turnsToRender.length - 1;
                    const isGeneratingTurn = isGenerating && isLastTurn;
                    return (
                      <TurnUnit
                        ...
                        onCreateBranch={(id, newContent) => createBranch(id, newContent, apiKey!, selectedModelId!, mergedConfig, actions)}
                        isMobile={isMobile}
                      />
                    );
                  });
          ...
          <div className={`shrink-0 ${isMobile ? 'p-2' : 'p-4'} pt-0`}>
      ...
      inspector={
        <>
          {replayState.isActive ? <ReplayInspector /> : <InspectorPanel />}
        </>
      }
    />
      <AnimatePresence>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
            ...
            >
              ...
              <div className="p-8 space-y-8">
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    <h3 className="text-lg font-bold text-white uppercase tracking-widest">Athanor Forja</h3>
