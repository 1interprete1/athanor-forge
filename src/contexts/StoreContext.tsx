...
import { ModelDefinition, InferenceConfig, ArtifactInstance, InfrastructureTier, QuotaInfo, Message, ChatSession, PerformanceEntry, AttachedFile, ExportConfig, DevMode, ExportProfile, ReplayState, ToastState } from '../types';
...
interface StoreActions {
  ...
  setExportConfig: (config: ExportConfig | ((prev: ExportConfig) => ExportConfig)) => void;
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const [exportConfig, setExportConfig] = useState<ExportConfig>(() => {
    const saved = localStorage.getItem('athanor_export_config');
    try {
      return saved ? JSON.parse(saved) : DEFAULT_EXPORT_CONFIG;
    } catch {
      return DEFAULT_EXPORT_CONFIG;
    }
  });

  const updateExportConfig = useCallback(<K extends keyof ExportConfig>(key: K, value: ExportConfig[K]) => {
    setExportConfig(prev => {
      const newConfig = { ...prev, [key]: value };
      localStorage.setItem('athanor_export_config', JSON.stringify(newConfig));
      return newConfig;
    });
  }, []);

  const [exportProfile, setExportProfileState] = useState<ExportProfile>(() => {
    const saved = localStorage.getItem('athanor_export_profile');
    return (saved as ExportProfile) || 'clean';
  });

  const setExportProfile = useCallback((profile: ExportProfile) => {
    setExportProfileState(profile);
    localStorage.setItem('athanor_export_profile', profile);
    
    let newConfig: ExportConfig | null = null;
    if (profile === 'clean') {
      newConfig = { includeTimestamp: false, includeModel: false, includeTokens: false, includeCost: false, includeRole: false, includeRaw: false };
    } else if (profile === 'debug') {
      newConfig = { includeTimestamp: true, includeModel: true, includeTokens: true, includeCost: false, includeRole: true, includeRaw: false };
    } else if (profile === 'audit') {
      newConfig = { includeTimestamp: true, includeModel: true, includeTokens: true, includeCost: true, includeRole: true, includeRaw: true };
    }
  ...
  const [devMode, setDevModeState] = useState<DevMode>(() => {
    const saved = localStorage.getItem('athanor_dev_mode');
    return (saved as DevMode) || 'simple';
  });

  const setDevMode = useCallback((mode: DevMode) => {
    setDevModeState(mode);
    localStorage.setItem('athanor_dev_mode', mode);
  }, []);
  ...
  const [activeSessionId, setActiveSessionId] = useState<string>(() => {
    const savedId = localStorage.getItem('athanor_active_session_id');
  ...
  useEffect(() => {
    localStorage.setItem('athanor_active_session_id', activeSessionId);
  }, [activeSessionId]);
  ...
  const nextTurn = useCallback(() => {
    setReplayState(prev => {
      ...
      const turns = groupMessagesIntoTurns(activeSession.messages);
      if (prev.currentTurnIndex >= turns.length - 1) return prev;
  ...
  const prevTurn = useCallback(() => {
    setReplayState(prev => {
      if (!prev.isActive) return prev;
      return { ...prev, currentTurnIndex: Math.max(0, prev.currentTurnIndex - 1) };
    });
  }, []);
  ...
  useEffect(() => {
    const groupId = startGroup("Athanor Forja: System Patch AF-001 Applied");
    addLog('System', 'success', "Bug Registry visibility issue resolved via Portal decoupling.", { 
      patch: 'AF-001', 
      method: 'React.createPortal',
  ...
  const [isMobile, setIsMobile] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      return isMobileUA || window.matchMedia('(max-width: 1024px)').matches || window.innerWidth < 1024;
    }
    return false;
  });
  ...
  useEffect(() => {
    const checkMobile = () => {
      const isMobileUA = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
      const mobile = isMobileUA || window.matchMedia('(max-width: 1024px)').matches || window.innerWidth < 1024;
      setIsMobile(mobile);
    };
  ...
  const [githubToken, setGithubTokenState] = useState(() => secretsConfig.getGithubToken() || '');

  const setGithubToken = useCallback((token: string) => {
    setGithubTokenState(token);
    secretsConfig.setGithubToken(token);
  }, []);
  ...
  const runContextAudit = useCallback(async () => {
    ...
    const groupId = startGroup(`Athanor Forja AF-173: ${isFree ? 'Frugal' : 'Chaos'} Engineering Module Deployed`);
    ...
    try {
      ...
      if (mismatch > 0) {
        ...
        console.warn("[BUG DETECTED] AF-MISMATCH - Context Integrity Mismatch detected during Pilot Audit.");
      } else {
  ...
  const value = useMemo(() => ({
    ...
    exportConfig, setExportConfig, updateExportConfig,
    exportProfile, setExportProfile,
    devMode, setDevMode,
    replayState, startReplay, stopReplay, nextTurn, prevTurn, togglePlay,
    ...
    models, activeModelId, activeModel, config, exportConfig, exportProfile, devMode, replayState, sessions, activeSessionId, isGenerating, metrics, estimatedTokens, artifacts, activeArtifactId, lastModelsUpdate, favoriteModels,
    isPilotActive, pilotGhostText, pilotTriggerSendMessage, tier, quota,
    updateConfig, updateExportConfig, setExportProfile, setDevMode, startReplay, stopReplay, nextTurn, prevTurn, togglePlay, resetConfig, toggleFavoriteModel, runContextAudit, setQuota,
