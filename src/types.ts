...
export type DevMode = 'simple' | 'advanced';
export type ExportProfile = 'clean' | 'debug' | 'audit' | 'custom';
...
export interface Message {
  ...
  timestampMs?: number; // Numeric timestamp for precise filtering
  cost?: number;
  tps?: number;
  ttft?: number;
  jsonMode?: boolean;
...
export interface AttachedFile {
  id: string;
  name: string;
  type: 'pdf' | 'text' | 'code' | 'json' | 'csv' | 'image';
  size: number;
  content: string | null;
